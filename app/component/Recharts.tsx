...
// Функция для форматирования текста с ** в жирный шрифт
const formatAIText = (text: string) => {
  ...
  // Разделяем на параграфы по переносам строк
...
// Функция для извлечения эмодзи и создания красивых блоков
const extractEmojiSections = (text: string) => {
  if (!text) return null;
  
  // Разделяем текст на секции по эмодзи
  ...
  return sections.map((section, index) => {
    ...
    const isEmojiSection = /[📊💡📈📉🎯⚠️✅]/.test(firstChar);
    
    if (isEmojiSection) {
      const emoji = firstChar;
...
export default function LineChartDemo() {
  ...
  const analyzeChartData = async () => {
    ...
    try {
      ...
      setAnalysisError(err.message || 'Ошибка при анализе данных');
  ...
  const getQuickStats = () => {
    ...
    chartData.chartData.forEach(chart => {
      ...
      if (parseFloat(growth) > 20) {
        ...
        stats.trends.push(`➡️ ${chart.name} стабилен: ${growth}%`);
      }
    });
  ...
  return (
    <div id="trends" className="w-full bg-white/5 backdrop-blur-lg rounded-3xl p-6 border border-white/10">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-white font-bold text-lg">
                {isAnalyzing ? 'AI Анализ...' : analysisError ? 'Анализ данных' : 'AI Анализ'}
              </h3>
              <button
            ...
            {isAnalyzing ? (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-gray-300">
                  ...
                  <p className="text-sm">AI анализирует данные графиков...</p>
                </div>
                <div className="text-xs text-gray-400">
                  Обнаружены тренды: {quickStats.trends.slice(0, 2).join(', ')}
                </div>
              </div>
            ) : aiAnalysis ? (
              <div className="space-y-3">
                {extractEmojiSections(aiAnalysis)}
              ...
              <div className="text-center py-4">
                ...
                <p className="text-gray-500 text-xs mt-1">
                  Нажмите "Обновить" для получения AI-анализа ваших графиков
                </p>
              </div>
            )}
          </div>
      ...
      {/* Добавляем стили для жирного текста */}
      <style jsx>{`
        strong {
          font-weight: 600;
          color: #ffffff;
